...
- `mcp` — home-grown tools (nearest vendor, overloaded zones, inventory) over ES with in-memory haversine fallback. These are still used by `MCPTools` internally; the **official Elastic MCP server** is now the ADK agent's primary tool layer via `MCPToolset` (M5 done).
