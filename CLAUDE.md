...
The Google Cloud **Agent Builder** requirement, satisfied via the **ADK** (`google-adk`). Defines a real `LlmAgent` (`arenapulse_coordinator`) with Gemini 3 as the model. Tools: (1) **official Elastic `McpToolset`** registered first when `ELASTIC_MCP_URL` is set — transport auto-detected from the URL (`SseConnectionParams` when it contains "sse", `StreamableHTTPConnectionParams` otherwise, e.g. Elastic Cloud `…/api/agent_builder/mcp`), with an `Authorization: ApiKey` header injected from `ELASTIC_API_KEY` — Gemini calls `elasticsearch_search` natively via the official Elastic MCP server; (2) **`find_nearby_vendors` `FunctionTool`** — local Elastic client wrapper, fallback when MCP server not set. Import path is `google.adk.tools.mcp_tool.mcp_toolset` (the `mcp_tool` package itself exports nothing); requires the `mcp` pip package. ADK session ids are unique per call (`evt-<event_id>-<uuid8>`) — replans reuse the event_id and `create_session` raises on duplicates. This is where all three hackathon pillars converge. Import-guarded and **optional**: `adk_available()` is True only when `USE_ADK` + `google-adk` installed + Gemini configured. `plan_via_adk()` returns a normalized plan or `None`. `PlanningAgent.plan()` tries paths in order — **ADK → direct Gemini JSON → heuristic** — and tags `plan["planner"]` with `adk`/`gemini`/`heuristic`. Nothing runs at import time, so boot stays zero-dep.
...
Pydantic-v2 `Settings` singleton (`settings`), env-var driven, configured via `model_config = SettingsConfigDict(case_sensitive=True, extra="ignore")`. Key flags: `SIMULATION_ACTIVE` (**set to `false` in `.env`** — see simulator note above), `SIMULATION_INTERVAL_SECONDS`, `DRY_RUN`, `CORS_ORIGINS`, `ML_MODEL_PATH`, `STRICT_RAG` (default true — RAG self-correction triggers on infeasible plans), `PLAN_EVAL_ENABLED` (default true — LLM-judge per pipeline), `GEMINI_EMBED_MODEL` + `EMBED_DIMS` (semantic RAG embeddings). Gemini: `GOOGLE_GENAI_USE_VERTEXAI`, `GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_LOCATION`, `GEMINI_MODEL` (**currently `gemini-3.1-flash-lite` in `.env`** — 500 RPD dev default; swap to `gemini-3-flash-preview` for judging if "Gemini 3" label matters; `gemini-3-flash` 404s on AI Studio), `GEMINI_API_KEY`. Agent: `USE_ADK` (default true — route planning through the ADK agent when available), `ELASTIC_MCP_URL` (optional endpoint of the official Elastic MCP server — SSE or StreamableHTTP, transport auto-detected from the URL; when set, ADK uses `McpToolset` as primary tool; leave empty to use local `find_nearby_vendors` FunctionTool), `ELASTIC_API_KEY` (also injected as `Authorization: ApiKey` header on MCP connections), `APPROVAL_REQUIRED` (default false), `APPROVAL_RESOURCE_THRESHOLD` (default 5000). See `backend/.env.example`. `STRICT_RAG` and `APPROVAL_REQUIRED` can be toggled at runtime via `POST /api/v1/status/settings`.
...
- `mcp` — home-grown tools (nearest vendor, overloaded zones, inventory) over ES with in-memory haversine fallback. These are still used by `MCPTools` internally; the **official Elastic MCP server** is now the ADK agent's primary tool layer via `McpToolset` when `ELASTIC_MCP_URL` is set (M5 done — Railway vars configured).
...
- **Pages**: `Landing`, `Dashboard`, `Platform`, `Capabilities`, `Operations` (Case Studies), `Process`, `Contact`, `SupplyHub` (B2B), `OperatorCenter` (Command Center), `GlobalAnalytics`, `SystemStatus`. All routed in `App.tsx`, plus a catch-all `*` route that redirects unknown URLs to `/` (previously a blank page). `AICopilot` is rendered globally in `App.tsx` (outside routes) so it floats on every page.
...
`run_demo_scenario()`: 5-event scripted cascade (~50s total): normal_flow → congestion → crowd_surge (South Gate — road closure triggers RAG self-correction) → security_alert → recovery. Each event has SoFi Stadium lat/lon. All random events from `simulator/events.py` also now include lat/lon so the ADK vendor-search tool always has coordinates. **Vendor coords must match event coords**: `simulator/vendors.py` `STADIUM_CENTER_LAT/LON` is the SoFi center (33.9534, -118.3392), same as the zone coords in `events.py`/`demo.py` — it previously pointed ~8 km away, so `find_nearby_vendors` always returned 0 within 1 km. Vendors regenerate (new UUIDs) on each boot and are re-ingested to ES, so the `vendors` index accumulates stale docs across boots; far-away ones never match geo queries but inflate counts.

### Deployment
- **Backend — Railway** — `backend/Dockerfile` (python:3.12-slim, installs `requirements.txt`, runs `uvicorn app.main:app` honoring `$PORT`). Root directory = `backend/`; Railway auto-detects the Dockerfile. **Deployed and live.** Set env vars in Railway Variables tab (see `vercel_railway_deployment_guide.md`). ML model not baked in — `SurgePredictor` falls back to heuristic if absent.
- **Frontend — Vercel** — `frontend/vercel.json` (Vite preset, SPA rewrite). **Deployed and live.** `VITE_API_BASE` set to Railway backend URL in Vercel project env vars (see `frontend/.env.example`).

## Known issues / tech debt
- **Elastic MCP server (M5 done)** — Official Elastic MCP server wired into ADK via `McpToolset` (import from `google.adk.tools.mcp_tool.mcp_toolset`; needs the `mcp` pip package). Transport auto-detected from `ELASTIC_MCP_URL`: contains "sse" → `SseConnectionParams`, otherwise `StreamableHTTPConnectionParams` (Elastic Cloud hosted: `https://<cluster>.es.io/api/agent_builder/mcp`). `ELASTIC_API_KEY` is sent as `Authorization: ApiKey` header. When set, the ADK agent registers the official toolset as its first tool (Gemini can call `elasticsearch_search` natively); `find_nearby_vendors` FunctionTool stays as fallback. Local Docker: `docker run -p 3001:3001 -e ES_URL=... -e ES_API_KEY=... docker.elastic.co/integrations/mcp-server-elasticsearch:latest` → `ELASTIC_MCP_URL=http://localhost:3001/sse`.
